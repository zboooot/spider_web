...
12. **当前工作分支**：`feature/restore-local-spider-ui`
    - 这个分支在 git pull 导致内容覆盖后重建，包含了所有本地开发内容
    - 提交到这个分支时不要混入远端 `main` 的多蜘蛛/多角色相关代码

13. **git pull 有覆盖风险**：
    - 远端 `main` 分支包含"多蜘蛛 V4"版本，与本分支架构不兼容
    - 拉取前先确认本地改动已提交或 stash
    - 如果不小心 pull 覆盖，可以用 `git stash pop stash@{0}` 恢复（autostash）

### 资源文件

14. **所有图片资源在 `src/assets/`**，通过 ES module `import` 引入，Vite 会自动内联为 base64

15. **文件名拼写要严格一致**，之前有过 `wrom02.png`（错误）vs `worm02.png`（正确）导致的 bug

16. **库存图标 canvas 尺寸**固定为 `48×48`（`index.html` 里 `width="48" height="48"`），如需修改图标大小，两处都要改
